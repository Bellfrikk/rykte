...
export async function velgGruppeOppsett (){
  ...
  .from('gruppeTabell')
  .select('navn,gruppeId,status') as { data: { navn: string, gruppeId: any,status:string }[] | null, error: any };

  if (error) console.error('feill ved henting av grupper: ' + error)
  else if (data) {
    data.forEach(gruppe => {
      if(gruppe.status === 'ny') nyGruppeKnapp(gruppe.navn, gruppe.gruppeId);
    });
  }
...
async function hentGruppeInfo (nyGruppeId:any) {
...
async function oppdaterSpelarNr(gruppe:number,spelarNr:number){
  ...
  const { error } = await supabase
    ...
    .eq('gruppeId', gruppe);
  if( error ) { console.error('feil ved oppdatering av gruppeinfo: '+error);  } 
}